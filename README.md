<div align="center">

<a href="https://github.com/mizesko">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=20&pause=1000&color=00B8FF&center=true&vCenter=true&width=600&lines=%3E_++Ol%C3%A1%2C+eu+sou+o+Jo%C3%A3o+Victor+;%3E_++T%C3%A9cnico+em+TI+%26+Tech+Enthusiast;%3E_++Estudante+de+Python+%26+Pentest" alt="Typing SVG" />
</a>

<br><br>
<img src="mizesko.gif" alt="Mizesko" width="250" />
<br><br>

<p align="center">
  <em>
    Técnico em Informática formado pela ETEC e apaixonado por tecnologia.<br>
    Atualmente em transição e aprendizado contínuo, trilhando o caminho do <b>Desenvolvimento Python</b> e da <b>Segurança Ofensiva</b>.
  </em>
</p>
┌──(mizesko㉿mizesko)-[~]

└─$ echo ''hello world :)'' 

</div>

---

### 🛡️ Segurança Cibernética & Estudos

Sou um estudante dedicado, focando na compreensão das vulnerabilidades de sistemas e em como mitigá-las. Meu objetivo é ir além dos laboratórios e entender o comportamento de aplicações reais.

* 🎓 **Formação:** Técnico em Informática (ETEC).
* 🔒 **Em Andamento:** Google Cybersecurity Professional Certificate (Coursera).
* 🧠 **Foco de Estudo:** Exploração de vulnerabilidades, superfícies de ataque e automação de segurança.
* ⚔️ **Laboratórios:** Praticante ativo em plataformas de CTF e ambientes controlados para aprimorar minhas habilidades ofensivas e defensivas.

---

### ⚙️ Mentalidade de Construtor

Acredito em unir segurança e desenvolvimento ágil. Não busco apenas quebrar sistemas, mas também construir ferramentas eficientes.

* 🚀 **Vibe-Coding:** Utilizo o poder da Inteligência Artificial como parceira no desenvolvimento diário para escrever códigos mais rápidos e inteligentes.
* 🧩 **Low-Code Orchestration:** Aplico práticas de Low-Code para estruturar e validar ideias de forma ágil, integrando serviços e automatizando fluxos de trabalho.

---

### 🛠️ Tecnologias e Ferramentas

<div align="center">

  <img src="https://skillicons.dev/icons?i=python,bash,linux,kali,windows,git,sqlite,htmx,supabase&theme=dark" alt="Skills" />

</div>



### 🌐 Conecte-se comigo

<div align="center">

  <a href="https://linkedin.com/in/mizesko" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="https://discordapp.com/users/mizesko" target="_blank">
    <img src="https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white" alt="Discord" />
  </a>
  <a href="https://portfolio-mizesko.vercel.app/" target="_blank">
    <img src="https://img.shields.io/badge/Portfólio-000000?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfólio Pessoal" />
  </a>
  <a href="mailto:mizeskowork16@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/E_mail-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>

<br><br>

  <img src="https://komarev.com/ghpvc/?username=mizesko&style=flat-square&color=00b8ff&label=Profile+Views" alt="Profile Views" />

</div>


# 📊 GitHub Stats:
![](https://github-readme-stats.shion.dev/api?username=mizesko&theme=transparent&hide_border=false&include_all_commits=true&count_private=true)<br/>
![](https://streak-stats.demolab.com/?user=mizesko&theme=transparent&hide_border=false)<br/>
![](https://github-readme-stats.shion.dev/api/top-langs/?username=mizesko&theme=transparent&hide_border=false&include_all_commits=true&count_private=true&layout=compact)

## 🏆 GitHub Trophies
![](https://github-profile-trophy.vercel.app/?username=mizesko&theme=radical&no-frame=true&no-bg=true&margin-w=4)

---

<!-- Растап -->
<picture>
<source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/mizesko/mizesko/output/pacman-contribution-graph-dark.svg">
<source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/mizesko/mizesko/output/pacman-contribution-graph.svg">
<img alt="pacman contribution graph" src="https://raw.githubusercontent.com/mizesko/mizesko/output/pacman-contribution-graph.svg">
</picture

---
